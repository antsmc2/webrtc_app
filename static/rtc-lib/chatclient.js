...
// session.
function setUsername(username, notify_peer) {
  myUsername = username;
  if(notify_peer)
    sendToServer({
      name: myUsername,
...
//      case "receiver-status":      // Received an updated user list
//        handleReceiveStatusMsg(msg);
//        break;
...
function handleReceiveStatusMsg(msg) {
  if(msg.status == 'ready') {
      return invite(msg.name);
  }
  else
     log('unknown status: ' + msg.status);
}
